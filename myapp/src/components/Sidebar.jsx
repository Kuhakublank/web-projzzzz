...
export default function Sidebar({ userType = "teacher" }) {
  ...
  return (
    <aside
      className={`min-h-screen bg-gradient-to-b from-rose-900 to-red-900 text-white flex flex-col
      transition-all duration-300
      ${isOpen ? "w-64" : "w-16"}`}
    >
      {/* Header */}
      <div className="flex items-center justify-between p-5 border-b border-rose-800">
        {isOpen && (
          <Link to={`/${userType}-dashboard`} className="text-2xl font-bold">
            LMS
          </Link>
        )}
